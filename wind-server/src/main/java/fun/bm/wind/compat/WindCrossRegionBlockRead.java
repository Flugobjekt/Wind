...
/**
 * Wind: serve a <em>read-only</em> block lookup from an already-loaded chunk when the calling
 ...
 */
public final class WindCrossRegionBlockRead {

    private static final Logger LOGGER = LogManager.getLogger(WindCrossRegionBlockRead.class);
    ...
    public static BlockState readIfResident(final ServerLevel level, final BlockPos pos) {
        ...
        try {
            ...
            LOGGER.warn("[Wind] Cross-region block read raced a palette update at {} in {}; "
     ...
     * {@link WindForeignWorldTicketUpdates}, without which that case is not merely slow but
     * actively harmful.
     *
    ...
    private static BlockState readByLoading(final ServerLevel level, final BlockPos pos) {
        ...
        try {
            ...
            LOGGER.warn("[Wind] Chunk load for a block read at {} in {} did not complete; "
    ...
    private static void reportLoad(final ServerLevel level) {
        final String where = io.papermc.paper.threadedregions.TickRegionScheduler.getCurrentRegion() != null
                ? "a region thread"
                : (WindStartupGlobalContext.isStartupThread() ? "the startup thread" : "the global region thread");
        if (!REPORTED.add("load " + where + " -> " + level.getWorld().getName())) {
            return;
        }
        LOGGER.info("""
                        [Wind] Loaded a chunk on {} to answer a block read in world '{}'
    ...
    private static void report(final ServerLevel level) {
        ...
        if (REPORTED.add(key)) {
            LOGGER.info("""
                    [Wind] Served a cross-region block read from a resident chunk: {}
