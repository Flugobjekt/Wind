...
/**
 * Wind: the one hook point for {@link WindEconomySerialization}'s service boundary.
 *
 ...
 * translation.
 */
public class WindServicesManager extends SimpleServicesManager {
    ...
    public <T> void register(@NotNull final Class<T> service, @NotNull final T provider,
        ...
        final Object wrapped = WindEconomySerialization.wrapServiceProvider(service, provider, plugin);
        if (wrapped != provider) {
