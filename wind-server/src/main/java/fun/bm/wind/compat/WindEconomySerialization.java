...
/**
 * Wind: per-account serialization for plugin economy ledgers that were written for a single
 ...
 * Unlike {@link WindCallerContextDispatch}, which fails <i>closed</i> (an unknown owner means
 ...
 *
 * <p>Kill switch: {@code compat-config.economy-serialization=false} - defaults to <b>on</b>, see
 * {@link #ENABLED} for why this one is not default-off.
 */
public final class WindEconomySerialization {
    ...
    public static Object wrapServiceProvider(final Class<?> service, final Object provider, final Plugin plugin) {
        ...
        try {
            ...
            LOGGER.info("[Wind] {}: serializing economy service {} per account - keyed on the "
                            + "service interface, not on the plugin; different accounts still run in parallel",
                    plugin.getName(), service.getName());
            return wrapped;
        } catch (final Throwable t) {
            LOGGER.warn("[Wind] economy service wrap failed (leaving provider unwrapped)", t);
            return provider;
        }
    }
    ...
    public static boolean isGuardedCommand(final Command command) {
        ...
        try {
            ...
            if (COMMANDS_LOGGED.add(name)) {
                LOGGER.info("[Wind] {}: running /{} under the exclusive economy lock - first hit "
