...
/**
 * Wind: give an embedded PaperLib copy a platform environment that matches this server.
 *
 * <h2>The gap</h2>
 ...
 *
 * <p>Kill switch: {@code compat-config.paper-lib-environment=false}, after which every embedded
 * copy keeps whatever handlers it selected for itself.
 */
public final class WindPaperLibAdapter {
    ...
    public static void install(final JavaPlugin plugin) {
        if (!CompatConfig.paperLibEnvironment) {
            return;
        }
        ...
        try {
            ...
            LOGGER.debug("[Wind] PaperLib environment scan skipped for {}", plugin.getName(), t);
        }
    }
    ...
    private static void installInto(final JavaPlugin plugin, final ClassLoader loader, final String binaryName) {
        ...
        try {
            ...
            LOGGER.warn("[Wind] {}: embedded PaperLib at {} would not hand over its environment", plugin.getName(), binaryName, t);
            return;
        }
        ...
        try {
            ...
            LOGGER.info(
                    "[Wind] {}: embedded PaperLib ({}) was using {} / {} - a synchronous pair it picked because"
                            ...
                            + " changed; disable with compat-config.paper-lib-environment=false.",
                    plugin.getName(),
                    binaryName,
            ...
            LOGGER.warn("[Wind] {}: could not install the platform environment into {}", plugin.getName(), binaryName, t);
        }
    }
    ...
    private abstract static class BaseHandler implements InvocationHandler {
        ...
        public final Object invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
            switch (method.getName()) {
                ...
                case "toString":
                    return "Wind platform environment handler";
                default:
                    break;
            }
