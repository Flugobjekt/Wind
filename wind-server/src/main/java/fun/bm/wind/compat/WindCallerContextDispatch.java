package fun.bm.wind.compat;

import com.mojang.logging.LogUtils;
import fun.bm.wind.config.modules.CompatConfig;
import fun.bm.wind.compat.WindExecutionProvenance.Owner;
...
/**
 * Wind: run a rejected sync Bukkit scheduler task on whatever the caller was already working on.
 *
 * <h2>The gap</h2>
 ...
 * {@link WindExecutionProvenance} one question - <i>on whose behalf is this thread running</i> -
 ...
 */
public final class WindCallerContextDispatch {
    ...
    private WindCallerContextDispatch() {
    }
    ...
    public static CraftTask tryDispatch(final CraftTask task, final long delay, final long period) {
        if (!CompatConfig.callerContextDispatch) {
            return null;
        }
        try {
            ...
            final Owner owner = WindExecutionProvenance.effectiveOwner();
            if (owner == null) {
            ...
            if (scheduled == null) {
                ...
                LOGGER.debug("[Wind] {}: sync scheduler task {} was not redispatched - {} is already gone",
                        plugin.getName(), taskClassOf(task), owner.description());
                return task;
            }

            WindDispatchedTasks.track(plugin, task, scheduled);
            ...
            LOGGER.warn("[Wind] caller-context scheduler dispatch failed "
    ...
    private static void runBody(final Plugin plugin, final CraftTask task, final ScheduledTask handle,
        ...
        if (task.isCancelled()) {
            WindDispatchedTasks.forget(plugin, task.getTaskId());
            handle.cancel();
            return;
        }
        try {
    ...
    private static void report(final Plugin plugin, final CraftTask task, final Owner owner,
        ...
        final String message = "[Wind] {}: sync scheduler task {} (delay={}, period={}) redispatched to {}. "
