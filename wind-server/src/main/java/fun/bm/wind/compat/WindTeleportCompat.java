...
/**
 * Wind: restores {@code Entity#teleport(Location, ...)} at the Bukkit API boundary.
 *
 * <h2>The gap</h2>
 ...
 *
 * <p>Kill switch: {@code compat-config.teleport-semantics=false} restores the stock Folia
 * behaviour, an unconditional throw from both {@code CraftEntity} and {@code CraftPlayer}.
 */
public final class WindTeleportCompat {
    ...
    private static void reportDeferred(final CraftEntity entity, final Location location) {
        ...
        try {
            ...
            LOGGER.info("""
                            [Wind] Entity#teleport deferred to the owning region
                              plugin   : {}
                              callsite : {}
                              entity   : {} in {}
                              target   : {} at {}, {}, {}
                              why      : the destination is in another region or world, its chunks are not
                                         ...
                                         (see WindTeleportHandover), so Folia cannot move the entity on
            ...
            LOGGER.warn("[Wind] teleport diagnostics failed (harmless)", t);
        }
    }
    ...
    private static String callsite() {
        return StackWalker.getInstance().walk(frames -> frames
                .filter(f -> !f.getClassName().startsWith("fun.bm.wind.compat."))
