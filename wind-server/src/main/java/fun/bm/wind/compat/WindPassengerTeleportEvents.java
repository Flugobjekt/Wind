...
import fun.bm.wind.config.modules.EventConfig;
...
/**
 * Wind: fires the Bukkit teleport events for passengers carried along by a teleported vehicle.
 *
 * <h2>The gap</h2>
 * Teleporting a vehicle moves its whole passenger tree - here and on Paper alike. But the events
 * only follow on Paper. {@link WindTeleportEvents} restores the event for the entity actually
 ...
 * {@code wind.compat.teleportEvents}, which governs the root entity's event.
 */
public final class WindPassengerTeleportEvents {
    ...
    public static void callForPassengers(final Entity vehicle, final ServerLevel destination,
        ...
        // plugins - same rule, and the same reasons, as WindTeleportEvents.
        if (!EventConfig.passengerTeleportEvents || cause == null || cause == TeleportCause.UNKNOWN) {
            return;
        }
