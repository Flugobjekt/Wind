...
/**
 * Wind: answer {@code Player#getRespawnLocation()} from any tick thread, instead of throwing
 ...
 * line the sibling {@link WindCrossRegionBlockRead} draws, for the same reason: only threads that
 ...
 */
public final class WindRespawnLocationLookup {
    ...
    private WindRespawnLocationLookup() {
    }
    ...
    private static void report(final ServerLevel level) {
        ...
        LOGGER.info("""
                        [Wind] Returned a player's stored respawn location without validating the spawn block.
                          callsite : {}
    ...
    private static String callsite() {
        for (final StackTraceElement frame : Thread.currentThread().getStackTrace()) {
            ...
            if (cls.startsWith("java.") || cls.startsWith("fun.bm.wind.compat.")
