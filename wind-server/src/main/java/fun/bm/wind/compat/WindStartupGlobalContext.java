...
/**
 * Wind: during startup the server bootstrap thread <em>is</em> the owner of global server state,
 ...
 *
 * <p>Kill switch: {@code compat-config.startup-global-context=false} restores the stock rejection.
 */
public final class WindStartupGlobalContext {

    private static final Logger LOGGER = LogManager.getLogger(WindStartupGlobalContext.class);
    ...
    private WindStartupGlobalContext() {
    }
    ...
    public static boolean allow(final String reason) {
        if (!CompatConfig.startupGlobalContext || !isStartupThread()) {
            return false;
        }
        if (REPORTED.add(reason)) {
            LOGGER.info("""
                            [Wind] Allowed a global-state call on the startup thread: {}
                            ...
                            reports true on this thread. Calls made after startup are rejected exactly as before.
                              disable : compat-config.startup-global-context=false""",
